...
class Player(pygame.sprite.Sprite):
    ...
    def update(self) -> None:
        ...
        if self.vitesse_y > 11:
            liste_platformes_touchées = pygame.sprite.spritecollide(self, self.platformes.platform_list, True)
        else:
            liste_platformes_touchées = pygame.sprite.spritecollide(self, self.platformes.platform_list, False)
        self.best_platform = len(self.platformes.rect_list)
        for platforme in liste_platformes_touchées:
            for i in range(len(self.platformes.rect_list)):
                if self.platformes.rect_list[i].rect.y - 4 <= self.rect.bottom <= self.platformes.rect_list[i].rect.y + 4:
...
def jeu():
    ...
    while True:
        ...
        # Dessine les ennemis
        if player.rect.y + 40 > hauteur_lave:
            partie_perdue(hauteur_lave, g, liste_de_sprites)
        étiquette('Niveau {}'.format(niveau), 10, 10)
